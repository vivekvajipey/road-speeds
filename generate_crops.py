...
if __name__ == '__main__':
    ...
    parser.add_argument('--in_tif_dirs', nargs='+', default=
        ...
        # ['data/sat_tifs/sat_img'])
    parser.add_argument('--in_gps_tracks', type=str, default='data/GPStracks_forDLcourse.csv')
    ...
    gps_tracks = pd.read_csv(args.in_gps_tracks)
    ...
    # stgid_round_gps_tracks = {}
    # for stgid_round, sub_df in gps_tracks.groupby('stgid_round'):
    #     stgid_round_gps_tracks[stgid_round] = sub_df

    tif_full_paths = []
    if args.valid_handles_yaml is None:
        for dir in args.in_tif_dirs:
            for path in os.listdir(dir):
                full_path = os.path.join(dir, path)
                tif_full_paths.append(full_path)
    else:
        valid_handles = set(yaml.safe_load(open(args.valid_handles_yaml, 'r')))
        for dir in args.in_tif_dirs:
            for path in os.listdir(dir):
                handle = path[:-4]
    ...
    # For keeping track of image ids and crop ids
    prev_lat_lons = {} # (lat_min, lon_min, lat_max, lon_max) -> [img_id, set of (mo, year) tuples]
    img_id_map = defaultdict(lambda: defaultdict(lambda: defaultdict(str))) # true_img_id -> year -> month -> tif_path
    img_ct_wrapper = [0]
    ...
    def job_task(tif_full_path, prev_lat_lons, img_ct_wrapper, img_id_map):
        tif_path = tif_full_path.split('/')[-1]
        img_path_base = tif_path[:-4]
        min_tracks = 1

        # path_round = img_path_base[1]
        # path_year_end = img_path_base[-1]
        # if path_round != path_year_end:
        #     img_path_base = f'R{path_year_end}{img_path_base[2:]}'

        # stgid_round = '_'.join(img_path_base.split('_')[:-2])
        # if stgid_round not in stgid_round_gps_tracks:
        #     print(f'Skipping {img_path_base} due to insufficient gps tracks (no stigd_round).')
        #     return None
        # stgid_round_df = stgid_round_gps_tracks[stgid_round]
        # if stgid_round_df.shape[0] < min_tracks:
        #     print(f'Skipping {img_path_base} due to insufficient gps tracks ({stgid_round_df.shape[0]}, stgid).')
        #     return None
    ...
    # img_dfs = Parallel(n_jobs=-1)(delayed(job_task)(path) for path in tif_full_paths)
    img_dfs = []
    for path in tif_full_paths:
        img_dfs.append(job_task(path, prev_lat_lons, img_ct_wrapper, img_id_map))
