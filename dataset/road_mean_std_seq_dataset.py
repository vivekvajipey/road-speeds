...
class RoadMeanSTDSeqDataset(torch.utils.data.Dataset):
    def __init__(
        self,
        csv_path: str,
        ...
        self.seq_idxs = []
        for crop_id, sub_df in self.df.groupby('crop_id'):
            df_2021 = sub_df[sub_df['year'] == 2021]
            df_2022 = sub_df[sub_df['year'] == 2022]
            df_2023 = sub_df[sub_df['year'] == 2023]
            if df_2021.empty or df_2022.empty or df_2023.empty:
                continue
            for i in df_2021.index:
                for j in df_2022.index:
                    for k in df_2023.index:
                        self.seq_idxs.append((i, j, k))


    def __len__(self):
        return len(self.seq_idxs)

    def _get_subitem(self, sub_idx):
        img_stack = []
        for image_col in self.image_cols:
            img_path = self.df.loc[sub_idx, image_col]
        ...
        tab_data = torch.tensor(self.tabular_info[sub_idx], dtype=torch.float32) # (n_feats,)

        mean_std_target = self.df.loc[sub_idx, ['speed_mean_norm', 'speed_std_norm']].to_numpy()
    ...
    def __getitem__(self, idx):
        idx_1, idx_2, idx_3 = self.seq_idxs[idx]
        img_1, tab_1, target_1 = self._get_subitem(idx_1)
        img_2, tab_2, target_2 = self._get_subitem(idx_2)
        img_3, tab_3, target_3 = self._get_subitem(idx_3)
        img_seq = torch.stack((img_1, img_2, img_3), dim=0)
        tab_seq = torch.stack((tab_1, tab_2, tab_3), dim=0)
        target_seq = torch.stack((target_1, target_2, target_3), dim=0)

        return img_seq, tab_seq, target_seq