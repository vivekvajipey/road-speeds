...
from models.resnet_concat_rnn import ResNetConcatLSTM
...
def main(args):
    ...
    name_label_norm = '-normlabel' if args.tabular_cols_file == 't_cols_norm.yaml' else ''
    name_dataset = 'years' if 'year' in args.csv_path else 'month'
    name_arch = 'seq' if args.use_sequence_model else 'tab'
    name_seq = f"-lstm{args.lstm_hidden_dim}_layers{args.lstm_layers}" if args.use_sequence_model else ''
    name_bands = ''
    name_norm = 'bn'
    name_hidden_dim = f"-hd{args.hidden_dim}" if args.hidden_dim else ""
    for img_col in args.image_cols:
        name_bands += (img_col.split('_'))[0]
    name_drop = '' if args.dropout == 0 else f'-drop{args.dropout}-'
    run_name = f"{args.model}-{name_arch}{name_seq}{name_pretrained}{name_label_norm}-{name_dataset}-{name_bands}-{name_norm}{name_hidden_dim}-E{args.n_epochs}-B{args.batch_size}-Lr{args.lr}-dec{args.weight_decay}"
    run_name += name_drop
    run_name += '-nostd'
    # run_name += '-jitter0.5_2-0.2-0.2-0.2'
    if args.use_wandb:
    ...
    for epoch in range(args.n_epochs):
        ...
        for batch_idx, (img_batch, tab_batch, label_batch) in enumerate(tqdm(train_loader)):
            ...
            loss_mean = criterion_mean(outs_mean, label_mean).mean()
            loss_std = criterion_std(outs_std, label_std).mean()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--use_sequence_model', action='store_true', default=False)
